...
class ActivityController extends Controller
{
    ...
    public function scheduleActivity()
    {
        ...
        usort($activities, function ($a, $b) use ($priority_map) {
            $priorityA = $priority_map[$a['activity_type']] ?? PHP_INT_MAX;
            $priorityB = $priority_map[$b['activity_type']] ?? PHP_INT_MAX;
        
            // Jika priority sama, urutkan berdasarkan deadline
        ...
        // Validasi Hard Constraints
        foreach ($activities as $activity) {
            if ($activity['duration'] <= 0) {
                throw new \Exception("Durasi aktivitas '{$activity['activity_name']}' tidak valid.");
            }
        }
        
        // Inisialisasi variabel jadwal
        $current_time = null; // Belum ada aktivitas
        $results = [];
        $total_waiting_time = 0;
        $total_late_completions = 0;
        
        foreach ($activities as $activity) {
            $deadline_time = strtotime($activity['deadline']);
            $duration_seconds = $activity['duration'] * 3600;
        
            // Jika aktivitas pertama, start_time dihitung dari deadline - durasi
            if (is_null($current_time)) {
                $start_time = $deadline_time - $duration_seconds;
            } else {
                // Aktivitas berikutnya dimulai segera setelah aktivitas sebelumnya selesai
                $start_time = min($current_time, $deadline_time - $duration_seconds);
            }
        
            $completion_time = $start_time + $duration_seconds;
        
            // Pastikan aktivitas tidak selesai setelah deadline
            if ($completion_time > $deadline_time) {
                throw new \Exception("Aktivitas '{$activity['activity_name']}' melewati deadline.");
            }
        
            $turnaround_time = $completion_time - $deadline_time;
            $waiting_time = max(0, $turnaround_time - $duration_seconds);
        
            // Evaluasi Soft Constraints
            $total_waiting_time += $waiting_time;
            if ($completion_time > $deadline_time) {
                $total_late_completions++;
            }
            
            $daysToDeadlinee = (new \DateTime($activity['deadline']))->diff(new \DateTime())->days;

            if ($daysToDeadlinee < 1) { // Critical deadline
                // $weight += 12;
                $urgents = 'critical';
            } elseif ($daysToDeadlinee <= 3) { // High urgency
                // $weight += 8;
                $urgents = 'high';;
            } elseif ($daysToDeadlinee <= 7) { // Medium urgency
                // $weight += 5;
                $urgents = 'medium';;
            } else { // Low urgency
                // $weight += 2;
            ...
            $results[] = [
                ...
                'completion_time' => date('H:i', $completion_time),
                'turnaround_time' => round($turnaround_time / 3600, 2),
                'waiting_time' => round($waiting_time / 3600, 2),
            ];
        
            $current_time = $completion_time; // Update waktu saat ini setelah aktivitas selesai
        }
        
        // Evaluasi jadwal
        // $results['evaluation'] = [
           // 'total_waiting_time' => round($total_waiting_time / 3600, 2),
          //  'total_late_completions' => $total_late_completions,
        //];
        ...
        foreach($results as $key => $valResult){
            if (!in_array($valResult['id'], $existingActivityIds)) {
                $fcfsModel->insert([
                    ...
                    'start_time' => $valResult['start_time'],
                    'completion_time' => $valResult['completion_time'],
                    'turnaround_time' => $valResult['turnaround_time'],
                    'waiting_time' => $valResult['waiting_time'],
                    'urgency' => $valResult['urgency'],
                    'created_at' => date('Y-m-d H:i:s'),
                    'updated_at' => date('Y-m-d H:i:s'),
                ]);
            } else {
    ...
    public function calculationResult()
    {
        ...
        $scheduleModel = new ScheduleModel();
        $schedules = $scheduleModel->where(['user_id' => session('user_id'), 'completion_stat' => 'pending'])->findAll();

        $hasilFcfs = new HasilFcfsModel();
        $fcfs = $hasilFcfs->where(['user_id' => session('user_id'), 'completion_stat' => 'pending'])->findAll();

...
class CronJob extends \Daycry\CronJob\Config\CronJob
{
    ...
    */
    public function init(Scheduler $schedule)
    {
        ...
        //     (new DashboardController())->scheduleActivity();
        // })->cron('* * * * *')->named('scheduleActivity');

        // Every one hour everyday
        //0 0-23/1 * * *
        $schedule->call(function () {
            (new DashboardController())->cronJobScheduleActivity();
            // Insert code for email
            // (new DashboardController())->emailNotification();
        })->cron('* * * * *')->named('cronJobScheduleActivity');

        $schedule->call(function () {
            (new DashboardController())->sendEmailReminder();
            // Insert code for email
            // (new DashboardController())->emailNotification();
        })->cron('* 8 * * *')->named('sendEmailReminder');

        // 
        // $schedule->call(function () {
        //     (new DashboardController())->emailNotification();
        // })->cron('* * * * *')->named('emailNotification');
    }
}
